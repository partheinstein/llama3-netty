...
public class Llama3 {
  ...
  static void runInteractive(Llama model, Sampler sampler, Options options) {
    ...
    while (true) {
      ...
      conversationTokens.addAll(
          chatFormat.encodeMessage(new ChatFormat.Message(ChatFormat.Role.USER, userText)));
      conversationTokens.addAll(
...
final class GGUF {
  ...
  private Pair<String, Object> readKeyValuePair(FileChannel fileChannel) throws IOException {
    ...
    assert key.codePoints()
        .allMatch(
            cp -> ('a' <= cp && cp <= 'z') || ('0' <= cp && cp <= '9') || cp == '_' || cp == '.');
  ...
  public static Llama loadModel(Path ggufPath, int contextLength) throws IOException {
    try (var ignored = Timer.log("Load LlaMa model")) {
      ...
      Pair<float[], float[]> ropeFreqs =
          RoPE.precomputeFreqsCis(config.contextLength, config.headSize, config.ropeTheta);
      ...
      Llama.Weights qw =
          new Llama.Weights(
              ...
              loadArrayOfFloatBuffer(
                  config.numberOfLayers, i -> tensorEntries.get("blk." + i + ".attn_norm.weight")),
              loadArrayOfQuantized(
                  config.numberOfLayers, i -> tensorEntries.get("blk." + i + ".attn_q.weight")),
              loadArrayOfQuantized(
                  config.numberOfLayers, i -> tensorEntries.get("blk." + i + ".attn_k.weight")),
              loadArrayOfQuantized(
                  config.numberOfLayers, i -> tensorEntries.get("blk." + i + ".attn_v.weight")),
              loadArrayOfQuantized(
                  config.numberOfLayers,
                  i -> tensorEntries.get("blk." + i + ".attn_output.weight")),
              loadArrayOfFloatBuffer(
                  config.numberOfLayers, i -> tensorEntries.get("blk." + i + ".ffn_norm.weight")),
              loadArrayOfQuantized(
                  config.numberOfLayers,
...
record Llama(Configuration configuration, Tokenizer tokenizer, Weights weights) {
  ...
  static void rmsnorm(
      FloatTensor out, FloatTensor x, FloatBuffer weight, int size, float rmsNormEps) {
    ...
    out.mapWithIndexInPlace(
        0, size, (value, index) -> weight.get(index) * (finalss * x.getFloat(index)));
  }
  ...
  static FloatTensor forward(Llama model, Llama.State state, int token, int position) {
    ...
    int kvMul =
        config.numberOfHeads
            / config.numberOfKeyValueHeads; // integer multiplier of the kv sharing in multiquery
      ...
      Parallel.parallelFor(
          ...
          h -> {
            ...
            for (int t = 0; t <= position; t++) {
              ...
              float score =
                  state.q.dot(qOffset, state.keyCache[curLayer], keyCacheOffset, headSize);
              score /= sqrtHeadSize;
   ...
   *     useful if state was kept across calls (chained generation). 0 implies run with no previous
   *     context.
   ...
   *     stderr}
   * @param onTokenGenerated callback, if non-null, it's called every time a token is inferred e.g.
   *     it's not called when ingesting prompt tokens
  ...
  public static List<Integer> generateTokens(
    ...
    for (int position = startPosition; position < maxTokens; ++position) {
      ...
      if (promptIndex < promptTokens.size()) {
        ...
        if (echo) {
          ...
          System.err.print(
              Tokenizer.replaceControlCharacters(model.tokenizer().decode(List.of(nextToken))));
        }
      } else {
    ...
    System.err.printf(
        "%n%.2f tokens/s (%d)%n", totalTokens / (elapsedNanos / 1_000_000_000.0), totalTokens);
...
class Tokenizer {
  ...
  /**
   * Unlike {@link #encodeOrdinary(String)}, this function handles special tokens. allowed_special:
   * can be "all"|"none"|"none_raise" or a custom set of special tokens if none_raise, then an error
   * is raised if any special token is encountered in text this is the default tiktoken behavior
   * right now as well any other behavior is either annoying, or a major footgun.
   */
...
enum GGMLType {
  ...
  Q5_K(
      2 * Float16.BYTES + ((GGMLType.QK_K / 16) / 8 * 6) + GGMLType.QK_K / 8 + GGMLType.QK_K / 2,
      GGMLType.QK_K),
...
final class Q4_0FloatTensor extends FloatTensor {
  ...
  public float getFloat(int index) {
    ...
    if (modIndex < GGMLType.Q4_0.getBlockSize() / 2) {
      quant =
          (byte)
              (memorySegment.get(ValueLayout.JAVA_BYTE, blockOffset + Float16.BYTES + modIndex)
                  & 0x0F);
    } else {
      quant =
          (byte)
  ...
  private static float vectorDot(
      Q4_0FloatTensor thiz, int thisOffset, FloatTensor that, int thatOffset, int size) {
    float result = 0f;
    int j = 0;
    ...
    for (;
      ...
      var wBytes =
          ByteVector.fromMemorySegment(
              B_SPECIES, thiz.memorySegment, blockOffset + Float16.BYTES, ByteOrder.LITTLE_ENDIAN);
      ...
      if (F_SPECIES.vectorBitSize() == 256) {
        ...
        for (int i = 0; i < 2; ++i) {
          ...
          var sum0 =
              that.getFloatVector(F_SPECIES, thatOffset + j + (i * 4 + 0) * F_SPECIES.length())
                  .mul(tmp.castShape(F_SPECIES, 0));
          var sum1 =
              that.getFloatVector(F_SPECIES, thatOffset + j + (i * 4 + 1) * F_SPECIES.length())
                  .mul(tmp.castShape(F_SPECIES, 1));
          var sum2 =
              that.getFloatVector(F_SPECIES, thatOffset + j + (i * 4 + 2) * F_SPECIES.length())
                  .mul(tmp.castShape(F_SPECIES, 2));
          var sum3 =
              that.getFloatVector(F_SPECIES, thatOffset + j + (i * 4 + 3) * F_SPECIES.length())
...
final class Q8_0FloatTensor extends FloatTensor {
  ...
  public float getFloat(int index) {
    ...
    byte quant =
        memorySegment.get(ValueLayout.JAVA_BYTE, blockOffset + Float16.BYTES + withinBlockIndex);
    float scale = Float.float16ToFloat(memorySegment.get(JAVA_SHORT_LE, blockOffset));
    return quant * scale;
  }
  ...
  private static float vectorDot(
      Q8_0FloatTensor thiz, int thisOffset, FloatTensor that, int thatOffset, int size) {
    float result = 0f;
    int j = 0;
        ...
        for (int i = 0; i < 2; ++i) {
          var wBytes =
              ByteVector.fromMemorySegment(
                  ...
                  ByteOrder.LITTLE_ENDIAN);
          var sum0 =
              that.getFloatVector(F_SPECIES, thatOffset + j + i * 16 + 0 * F_SPECIES.length())
                  .mul(wBytes.castShape(F_SPECIES, 0));
          var sum1 =
              that.getFloatVector(F_SPECIES, thatOffset + j + i * 16 + 1 * F_SPECIES.length())
                  .mul(wBytes.castShape(F_SPECIES, 1));
          var sum2 =
              that.getFloatVector(F_SPECIES, thatOffset + j + i * 16 + 2 * F_SPECIES.length())
                  .mul(wBytes.castShape(F_SPECIES, 2));
          var sum3 =
              that.getFloatVector(F_SPECIES, thatOffset + j + i * 16 + 3 * F_SPECIES.length())
                  .mul(wBytes.castShape(F_SPECIES, 3));
          val = sum0.add(sum1).add(sum2).add(sum3).fma(wScale, val);
        }
      } else {
...
class ChatFormat {
  ...
  public List<Integer> encodeDialogPrompt(
      boolean appendAssistantTurn, List<ChatFormat.Message> dialog) {
    List<Integer> tokens = new ArrayList<>();
